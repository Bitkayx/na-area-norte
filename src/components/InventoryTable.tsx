...
export default function InventoryTable({ items }) {
  ...
  return (
    <div
      className="w-full p-3 sm:p-8 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm mt-4 sm:mt-8 overflow-hidden"
      key={category}
    >
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-200 dark:border-slate-600">
              <th className="text-left py-2.5 sm:py-3 px-1 sm:px-4 text-xs font-bold uppercase text-slate-400">
                Descripción
              </th>
              <th className="text-left py-2.5 sm:py-3 px-1 sm:px-4 text-xs font-bold uppercase text-slate-400">
                Código
          ...
          <tbody>
            {filteredItems.map((item, index) => (
              ...
              >
                <td className="py-2.5 sm:py-3 px-1 sm:px-4 text-xs sm:text-sm text-slate-700 dark:text-slate-200">
                  {item.description}
                </td>
                <td className="py-2.5 sm:py-3 px-1 sm:px-4 text-xs sm:text-sm text-slate-500">
                  {item.code || "-"}
                </td>
                <td className="py-2.5 sm:py-3 px-1 sm:px-4 text-xs sm:text-sm font-medium text-primary">
